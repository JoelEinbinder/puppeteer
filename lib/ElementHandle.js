...
class ElementHandle extends JSHandle {
  ...
  async boundingBox() {
    let result = null;
    try {
      result = await this._client.send('DOM.getBoxModel', {
        objectId: this._remoteObject.objectId
      });
    } catch (protoError) {
      debugError(protoError);

      const noElementError = await this.executionContext().evaluate(element => {
        if (!element.isConnected)
          return 'Node is detached from document';
        if (element.nodeType !== Node.ELEMENT_NODE)
          return 'Node is not of type HTMLElement';
      ...
      if (noElementError)
        throw new Error(noElementError);
    }
  ...
  async screenshot(options = {}) {
    ...
    if (boundingBox.width > viewport.width || boundingBox.height > viewport.height) {
      ...
      await this._page.setViewport(newViewport);
